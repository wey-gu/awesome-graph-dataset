...
def ng_load(ng_exec: BaseExecutor, args: NGLoadArgs, verbose: bool = False):
    """
    ...
    Args:
        ng_exec (BaseExecutor): NebulaGraph Executor
        args (NGLoadDataArgsModel): Arguments for loading data
        tqdm (Optional[tqdm], optional): tqdm progress bar. Defaults to None which shows no bar. Use tqdm.auto.tqdm for auto detection.
    """

    # Check if space is specified
    space = args.space
    ng_exec.execute(f"USE `{space}`")
    # Inspect space to get Vid Type
    r = ng_exec.execute(f"DESC SPACE `{space}`")
    try:
    ...
    r = ng_exec.execute(f"DESCRIBE {DESC_TYPE} `{DESC_TARGET}`")
    ...
    if args.tag:
        ...
        iteration = range(0, len(vertex_data), batch_size)
        if verbose:
            iteration = tqdm(iteration, desc="Loading Vertices")
        for i in iteration:
        ...
        iteration = range(0, len(edge_data), batch_size)
        if verbose:
            iteration = tqdm(iteration, desc="Loading Edges")
        for i in iteration:
...
if __name__ == "__main__":
    ...
    def args_load(line: str):
        ...
        return NGLoadArgs.model_validate(kv)

    test = """
# https://graph-hub.siwei.io/en/latest/datasets/shareholding
%ng_load  --source datasets/shareholding/tiny/person.csv --tag person --vid 0 --props 1:name --space shareholding
%ng_load  --source datasets/shareholding/tiny/corp.csv --tag corp --vid 0 --props 1:name --space shareholding
%ng_load  --source datasets/shareholding/tiny/person_corp_role.csv --edge role_as --src 0 --dst 1 --props 2:role  --space shareholding
%ng_load  --source datasets/shareholding/tiny/corp_rel.csv --edge is_branch_of --src 0 --dst 1   --space shareholding
%ng_load  --source datasets/shareholding/tiny/corp_share.csv --edge hold_share --src 0 --dst 1 --props 2:share  --space shareholding
%ng_load  --source datasets/shareholding/tiny/person_corp_share.csv --edge hold_share --src 0 --dst 1 --props 2:share  --space shareholding
%ng_load  --source datasets/shareholding/tiny/person_rel.csv --edge reletive_with --src 0 --dst 1 --props 2:degree  --space shareholding
"""
    ng_exec = conn_pool.get_session("root", "nebula")
    for line in test.split("\n"):
        if line.startswith("%ng_load"):
            args = args_load(line[9:])
            ng_load(ng_exec, args, verbose=True)
